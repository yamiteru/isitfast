import { object as zod_object, string as zod_string, number as zod_number } from "zod";
import { object as valibot_object, string as valibot_string, number as valibot_number, parse, minLength, maxLength, minValue, maxValue } from "valibot";

const assert = (fn) => (value) => {
	if(fn(value) === false) throw void 0;
};

const rangeValue = (min, max) => assert((v) => v >= min && v <= max);
const rangeLength = (min, max) => assert((v) => v.length >= min && v.length <= max);

export const both = (fn1, fn2) => (value) => (fn1(value), fn2(value));

export const object = (schema) => {
	const keys = Object.keys(schema);
	const length = keys.length;

	return (value) => {
		for(let i = 0; i < length; ++i) {
			schema[keys[i]](value[keys[i]]);
		}
	};
};

export const tString = (v) => assert(typeof v === "string");
export const tNumber = (v) => assert(typeof v === "number");

const validator_user = object({
  name: tString,
  password: both(tString, rangeLength(8, 16)),
  age: both(tNumber, rangeValue(0, 150)),
});


export const $validator = {
  $generator: function() {
    return {
      name: "Yamiteru",
      password: "Test123456",
      age: 26
    };
  },
  $function: function(value, blackbox) {
    blackbox(validator_user(value));
  }
};

const zod_user = zod_object({
  name: zod_string(),
  password: zod_string().min(8).max(16),
  age: zod_number().min(0).max(150)
});

export const $zod = {
  $generator: function() {
    return {
      name: "Yamiteru",
      password: "Test123456",
      age: 26
    };
  },
  $function: function(value, blackbox) {
    blackbox(zod_user.parse(value));
  }
};

const valibot_user = valibot_object({
  name: valibot_string(),
  password: valibot_string([minLength(8), maxLength(16)]),
  age: valibot_number([minValue(0), maxValue(150)]),
});

export const $valibot = {
  $generator: function() {
    return {
      name: "Yamiteru",
      password: "Test123456",
      age: 26
    };
  },
  $function: function(value, blackbox) {
    blackbox(parse(valibot_user, value));
  }
};
