...
import { getOffset, isNumberFluke, mem, now } from "@utils";
import {
  collectGarbage,
  sample,
  iterationStart,
  ...
  while (true) {
    ...
    if (iterations && !(iterations % SAMPLE_SIZE)) {
      ...
      const offset = getOffset(
        ARRAY_CHUNK.slice(
          CHUNK_SIZE,
          isOverSampleSize ? CHUNK_SIZE * 2 : CHUNK_SIZE + iterations,
        ),
        iterations,
      );
    ...
    const isFluke = isNumberFluke(
      ARRAY_CHUNK.slice(0, isOverSampleSize ? CHUNK_SIZE : iterations),
      iterations,
      data,
    );
    const time = now();
